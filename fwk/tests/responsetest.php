...
class ResponseTest extends \PHPUnit_Framework_TestCase
{
  ...
  */
  public function testRender($code, $body, $headers) 
  {
    ...
    $statusCodeMsgs = $this->_getProperty('fwk\Fwk_Response', 'responseCodes')->getValue($response);

    foreach ($headers as $key => $value) {
      $this->assertContains($key.":".$value, $headersList);
    }
    if (isset($statusCodeMsgs[$code])) {
      $this->assertContains($code." ".$statusCodeMsgs[$code], $headersList);
    }
