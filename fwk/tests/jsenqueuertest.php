...
class JsEnqueuerTest extends \PHPUnit_Framework_TestCase
{
  ...
  public function testPrivateConstructor() {
    $class = new \ReflectionClass('fwk\Fwk_JsEnqueuer');
  ...
  public function testGetInstance() {
    $enqueuer = fwk\Fwk_JsEnqueuer::getInstance();
    $className = get_class($enqueuer);
    
    $this->assertEquals('fwk\Fwk_JsEnqueuer', $className, 'Invalid class created');

    $enqueuer2 = fwk\Fwk_JsEnqueuer::getInstance();
  ...
  public function testEnqueue() {
    ...
    foreach ($scripts as $script) {
      ...
      $enqueuedScripts = $this->_getProperty('fwk\Fwk_JsEnqueuer', 'chunks')->getValue($enqueuer);
