...
class cache
{
  ...
  public static function getMulti($keysFamilies, $Parameters)
  {
    ...
    foreach($keysFamilies as $keyFamily)
    {
      $keyFamily = strtoupper($keyFamily);
      $cacheSource = self::getCacheSources($keyFamily);
      $keyParams = $Parameters[$counter]->build();
      $useGlobals = false;
      $useAPC = false;
...
class APCCache
{
  ...
  public static function set($keyFamily, $keyParameters, $object)
  {
    ...
    return apc_add($keyFamily."_".$keyParameters, $object, $cacheTime);
  }

  public static function get($keyFamily, $keyParameters)
  {
    $object = apc_fetch($keyFamily."_".$keyParameters, $result);
    if ($result)
      return $object;
    else
      return null;
  ...
  {
    return apc_delete($keyFamily."_".$keyParameters);
  }
}
...
{
  //HOST, LOCATION CODE, STATUS => Is always formed using parámeters in the order as they are in this array
  //Translation Section, Translation Key, Translation Variation => For translations
  public static $knownParameters = array("HST", "LOC", "ST", "TXVAR", "TXSEC", "TXKEY");
  private $properties = array();

  public function __set($paramName, $value)
  {
    if (in_array($paramName, self::$knownParameters))
    {
  ...
  {
    $keyParameters = "";
    foreach(self::$knownParameters as $paramType)
    {
      if (isset($this->properties[$paramType]))
      {
        $keyParameters = $paramType."-".$this->properties[$paramType];
      }
    }
